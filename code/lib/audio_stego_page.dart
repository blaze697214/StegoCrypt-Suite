...
class _AudioStegoPageState extends State<AudioStegoPage> {
  ...
  Future<void> _pickAudio() async {
    try {
      ...
      _showLongErrorDialog('Failed to pick audio', e.toString());
    }
  }
        ...
        if (stdoutText.isEmpty) {
          ...
          try {
            ...
            if ((json['success'] == true || json['status'] == 'success') &&
                json.containsKey('audio_data')) {
              final String base64Audio = json['audio_data'];
              final Uint8List audioBytes = base64Decode(base64Audio);
              setState(() {
                _encodedAudioBytes = audioBytes;
                _outputFilename = json['filename'] ?? 'encoded_audio.wav';
              });
  ...
  Future<void> _decodeMessage() async {
    if (_selectedAudioFile == null) {
      _showSnack('Please select an audio file to decode',
          backgroundColor: Colors.orange);
      return;
    }
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    _outputFilename ?? 'encoded_audio.wav',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: CyberTheme.bodyMedium.copyWith(
                      color: isDark ? Colors.white : Colors.black87,
                ...
                CyberButton(
                  ...
                  onPressed: () async {
                    try {
                      if (_encodedAudioBytes != null) {
                        final String? outputPath =
                            await FilePicker.platform.saveFile(
                          dialogTitle: 'Please select an output file:',
                          fileName: _outputFilename ?? 'stego_audio.wav',
                        );

                        if (outputPath != null) {
                          final file = File(outputPath);
                          await file.writeAsBytes(_encodedAudioBytes!);
