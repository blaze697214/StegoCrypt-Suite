...
class _TextStegoPageState extends State<TextStegoPage> {
  String? _selectedTextPath;
  String? _outputTextPath;
  ...
  Future<void> _pickText() async {
    try {
      ...
      if (result != null && result.files.single.path != null) {
        setState(() {
          _selectedTextPath = result.files.single.path!;
          _outputTextPath = null;
        });
      }
    } catch (e) {
  ...
  Future<void> _encodeMessage() async {
    if (_selectedTextPath == null || _messageController.text.isEmpty) return;
    ...
    setState(() {
      _isEncoding = false;
      _outputTextPath = _selectedTextPath!.replaceAll(RegExp(r'\.[^.]+$'), '') + '_stego.txt';
    });
  ...
  Future<void> _decodeMessage() async {
    if (_selectedTextPath == null) return;
    ...
    setState(() {
      _isDecoding = false;
      _messageController.text = 'This is a decoded secret message from the text file!';
    });
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 12),
              if (_selectedTextPath != null)
                Expanded(
                  child: Text(
                    _selectedTextPath!.split('/').last,
                    maxLines: 1,
